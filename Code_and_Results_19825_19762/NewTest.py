...
def eval():
    ...
    if not upscale_only:  # Otherwise the print will error on '-u'
        print(f"Avg PSNR Predicted of {args.file_list} = ", avg_psnr_predicted/count)
        # print("Avg SSIM Predicted = ", avg_ssim_predicted/count)

def save_img(img, img_name, pred_flag):
    if pred_flag:
        folder_dir = 'predicted'
    else:
        folder_dir = 'target'
    save_img = img.squeeze().clamp(0, 1).numpy().transpose(1,2,0)

    # save img
    save_dir=os.path.join(args.output, args.data_dir, os.path.splitext(args.file_list)[0]+'_'+str(args.upscale_factor)+'x',folder_dir)
    ...
    try:
        ...
        save_fn = save_dir +'/'+ f'{img_name:03d}'+'.png'
    except TypeError:
