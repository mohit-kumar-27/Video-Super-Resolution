...
class Model_loss(nn.Module):
    ...
    def forward(self, hr_est, hr_img, idx):
        # Perception Loss
        perception_loss = self.mse_loss(self.loss_network(hr_est), self.loss_network(hr_img))
        # Image Loss
        l2_loss = self.mse_loss(hr_est, hr_img)
        # TV Loss
        tv_loss = self.tv_loss(hr_est)
        # MAE loss
        l1_loss = self.mae_loss(hr_est, hr_img)

        return 0.5*l1_loss + 0.5*l2_loss + 0.006 * perception_loss + 2e-8 * tv_loss
